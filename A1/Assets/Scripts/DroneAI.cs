...
public class DroneAI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Node goal_node = new Node(goal_pos);
    ...
    private void FixedUpdate()
    {
        ...
        Vector3 acceleration = next_pos - drone_pos - m_Drone_rb.velocity;
        // Vector3 acceleration = 0.4f*(next_pos - drone_pos) - 0.2f*m_Drone_rb.velocity * (1f - 5f / (m_Drone_rb.velocity.magnitude+0.01f));
        ...
        for (int idx = pathpoint_index; idx < my_path.Count; idx++)
        {
            ...
            if (drone_pathpoint_dis < min_dis && idx - pathpoint_index <= 4)
            {
        ...
        if (Vector3.Distance(drone_pos, goal_pos) <= 3f)
        {
            Debug.Log("Stop!");
            m_Drone.Move(0f, 0f);
            stop = true;
        ...
        if (Physics.Raycast(transform.position + transform.up, transform.TransformDirection(Vector3.forward), out hit, 5) ||
            Physics.Raycast(transform.position + transform.right, transform.TransformDirection(Vector3.right), out hit, 5))
        {
        ...
        else if (is_hit)
        {
            ...
            else if ((Time.time - hit_time) < 4 && m_Drone_rb.velocity.magnitude > 0.1)
            {
                m_Drone.Move(0f, 0f);
            }
            else
        ...
        {
            if (Vector3.Distance(drone_pos, goal_pos) <= 1f)
            {
                Debug.Log("Stop!");
                m_Drone.Move(0f, 0f);
                stop = true;
            ...
            else if (Vector3.Distance(drone_pos, goal_pos) <= goalThreshold)
            {
        ...
        {
            //Vector3.Project(my_path[pathpoint_index].pos- my_path[pathpoint_index-1].pos,drone_pos_cur)
            //if (dist_to_next
            m_Drone.Move(acceleration.x, acceleration.z);
        }*/
        /*
